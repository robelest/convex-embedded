...
import { endUiMark, markUiClick, useTimeUiUpdate } from "@/src/ui-timing";
...
export function IssueDetail({ issueId, onDismiss }: IssueDetailProps) {
  ...
  const issue = useQuery(api.issues.detail, { issueId });

  // Defensive flush — if the sheet unmounts while title is still being edited
  // (e.g., user swipes down fast), persist the latest draft.
  ...
  const handleDeleteIssue = useCallback(async () => {
    ...
    onDismiss();
  }, [client, confirmDelete, issue, onDismiss]);
