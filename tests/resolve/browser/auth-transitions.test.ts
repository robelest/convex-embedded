...
describe("auth state transitions", () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(EmbeddedRuntime.prototype, "queryDirect").mockImplementation(
    ...
    vi.spyOn(EmbeddedRuntime.prototype, "mutationDirect").mockResolvedValue(
      null,
    );
  });
  ...
  it("transitions to identityMismatch when identity changes with pending replay work", async () => {
    ...
    const queryDirectSpy = vi.spyOn(EmbeddedRuntime.prototype, "queryDirect");
    queryDirectSpy.mockImplementation(async function (path: string) {
      if (path === "_system:authStateGetActive") {
        return null;
      }
      if (path === "_system:pendingListIdentityKeys") {
