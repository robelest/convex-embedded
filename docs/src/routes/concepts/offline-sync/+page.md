...
- Reads collection-tail metadata to determine whether incremental diffing is
  still valid.
- For incremental mode, reads the latest stored delta for each requested
  document from the embedded component.
- For full fallback mode, pages through stored live-state rows using a cursor.
...
- Returns either incremental diff rows or full document rows.

The resolve wire format includes a paged full-mode branch:
...
When `mode === "full"`, the client keeps requesting pages with `fullCursor`
until `isDone === true`, then ingests the completed snapshot in one pass.

### 3. Apply Diffs Locally

For incremental responses, each document that has a non-empty diff is applied to
the local Yjs document. For full responses, the client buffers all pages before
replacing the local table snapshot.
...
For full fallback, the server reconstructs full documents from the stored Yjs
live state plus the recorded source document `_creationTime`.
