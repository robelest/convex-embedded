...
becomes a local-first engine. Reads are always local and instant. Writes commit
to the embedded database first, then replay to the remote Convex deployment in
the background. When the client goes offline and comes back, a CRDT resolve pass
merges any state that diverged.
...
| `ready`      | All tables are resolved and reactive subscriptions are active. Normal steady state.                                                     |
...
3. **Replay** (when online) -- the queue drains serially. Each mutation is
   forwarded to the remote `ConvexClient` with ID translation: local document
   IDs (generated by the embedded runtime) are mapped to remote IDs (generated
   by the Convex backend) via the `IdMap`.
4. **Confirmation** -- once the remote mutation succeeds, the pending entry is
   removed from the queue and the ID mapping is stored.
...
each ready table. These reactive subscriptions keep local state fresh with
changes from other clients in real time. When new results arrive, they are
...
The pending mutation queue is stored in the embedded database (via
`_system:pendingPush` / `_system:pendingPop` system functions). This means:
